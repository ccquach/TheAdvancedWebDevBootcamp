d3.queue()
  ...
  .awaitAll(function(error, data) {
    ...
    function formatAllData(data) {
      ...
      var allData = [];
      data.reduce((acc, next) => acc.concat(next)).forEach(obj => {
        for (var year in obj) {
          if (parseInt(year)) {
            allData.push({
              region: obj.region,
      ...
      // group data by year, then region, then indicator
      var yearObj =
        d3.nest()
          .key(d => d.year)
          .key(d => d.region)
          .key(d => d.indicator.split(" ")[0].replace(",","").toLowerCase())
          .rollup(v => v[0].amount)
          .object(allData);
      ...
      for (year in yearObj) {
        var yearArray = [];
        for (key in yearObj[year]) {
          var vals = Object.values(yearObj[year][key]);
          if (!vals.includes(null)) {
            var newObj = { region: key };
            yearArray.push(Object.assign(newObj, yearObj[year][key]));
          }
        }
        // remove years with no data for any regions
