const mapPromise = d3.json("//unpkg.com/world-atlas@1/world/50m.json");
const dataPromise = d3.csv("./data/all_data.csv", function(row) {
  return {
...
.then(function(allData) {
  const mapData = allData[0];
  const data = allData[1];

  var extremeYears = d3.extent(data, d => d.year);
  var currentYear = extremeYears[0];
  var currentDataType = d3.select('input[name="data-type"]:checked').property("value");
